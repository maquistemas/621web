 ...
 */
$.navAsAjax = true;
console.log("apps::begin::1");
var i = 0;
if ($.navAsAjax) {
    // fire this on page load if nav exists
    if ($('nav').length) {
        checkURL();
        console.log("Primera carga del aplicativo1...");
    }
    ;
    $(document).on('click', 'nav a[href!="#"]', function (e) {
        console.log("Primera carga del aplicativo---1");
        e.preventDefault();
    ...
    $(document).on('click', 'div a.btn[href!="#"]', function (e) {
        e.preventDefault();
        var $this = $(e.currentTarget);
        window.location.hash = $this.attr('href');
    });
    ...
    $(document).on('click', 'address a[href!="#"]', function (e) {
        e.preventDefault();
        $(".Boton-Bandeja").addClass("show");
        $("#btnPaso02").trigger("click");
...
function checkURL() {
    //======================================================================
    // Código adicionado común a todos los formularios.
    // Permite asegurar la vigencia del token.
    console.log("app::checkURL::iniciando...");
    var token = sessionStorage.getItem('token');
    console.log("app::checkURL::token sessionStorage: " + token);

    if ((token == null) || (token == "null") || (token == "")) {
        console.log("app::checkURL, error token NO Encontrado!");
        $('#modalErrorIdCache').modal('show');
        // var token_hidden = $('#token', window.parent.document).text();
        // console.log("app::checkURL::token inicial: " + token_hidden);
        // token = token_hidden;
        // if (token == null) {
        //     console.log("app::checkURL::token invalido");
        //     //$('#modalErrorIdCache').modal('show');
        ...
        //     console.log("app::checkURL::token inicializado");
        // }
    }
    else {
        console.log("app::checkURL::token encontrado");
    }
    //======================================================================

    var url = location.hash.replace(/^#/, '');
    console.log("app::checkURL::url:"+url);
    container = $('#content');
    if (url) {
        ...
        //window.location.hash = 'buscarformularios.html'; ///$this.attr('href');
        console.log("app::checkURL::buscar!");
    }
}
// LOAD AJAX PAGES
function loadURL(url, container) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var m = data.replace(/<sscript.*?>([\w\W\d\D\s\S\0\n\f\r\t\v\b\B]*?)<\/sscript>/gi, '');
            var m = m.replace(/<head.*?>([\w\W\d\D\s\S\0\n\f\r\t\v\b\B]*?)<\/head>/gi, '');
            container.html(m);
        },
