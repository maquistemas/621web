var formulario0697 = (function () {
    var m_nameJavascript = "formulario0697.js";
    var m_idFormulario = "0697";
    ...
    _activarGuardadoAutomatico = function () {
        ...
        var idTimeOutTask = setTimeout("formulario0697.activarGuardadoAutomatico()", 60000); // auto salva cada minuto
    ...
    _obtenerPagosPrevios = function (showModal, iniciarCalculoInteresPagoPrevios, codigoFormulario, codigoTributo) {
        ...
        var periodo = $('*[data-casilla="7"]').val();
        var params = _formatearPeriodoaCadena(periodo) + '/' + m_idFormulario + '/010504';
        ...
        if (periodo != "") {
            ...
            if (data != null) {
                ...
                if (caracterfinal == "3" && ctnValCasilla895 == comunLibreria.getEnumTipoDeclaracion().ORIGINAL.value) {
                    ...
                    // var enumTipoDeclara = comunLibreria.getEnumTipoDeclaracion();
                    // $('*[data-casilla="895"]').empty();
                    // //validacion de fechavencimiento
                    // var elemento = data.jsonPeriodoBean.fechaVencimiento.split('/');
                    // var anio = elemento[2];
                    // var mes = elemento[1];
        ...
        if (caracterfinal == "3" && total==3) {
            ...
            if (control895.val()==comunLibreria.getEnumTipoDeclaracion().SUSITUTORIA.value) {
                ...
                if (moment(fechaPresentacion).isAfter(fechaVencimiento)) {
                    formulario0697.mostrarMensajeGeneral(comunMensajes.getMensaje("DECLAFECHMAYOR"));
                ...
                if (moment(fechaPresentacion).isBefore(fechaVencimiento)) {
                    formulario0697.mostrarMensajeGeneral(comunMensajes.getMensaje("DECLAFECHMENOR"));
                    control895.val(comunLibreria.getEnumTipoDeclaracion().ORIGINAL.value)
                }
            }
        }
...
$(document).ready(function () {
    ...
    localStorage.setItem("ventana", "f0697");

    var form0697 = formulario0697;

    form0697.setearFechaDefecto();
    ...
    console.log("Invocando parametria 0697");
    var _dataParametriaInicial = comunServiciosControlador.obtenerParametria(form0697.obtenerIdFormulario(), form0697.obtenerVersionFormulario());
    ...
    if (_dataParametriaInicial != null) {
        console.log("0697:decorator....");
        comunBandeja.setearFormularioActivo("f0697.html", _dataParametriaInicial.desFor);
        form0697.myIntegrador.parsearDecoracionCasillas(_dataParametriaInicial);

        if (form0697.myIntegrador.validarVigenciaFormularios(_dataParametriaInicial) == false) {
        ...
        form0697.muestraFormularioFrecuente();
    }

    $('*[data-casilla="7"]').change(function () {
        console.log("evento cambio del periodo 697");
    ...
    function iniciarProcesoObtencionDatos() {
        ...
        try {
            ...
            form0697.myIntegrador.setearActivacionCambioCasillas("false==" + periodoIngresado);
            var params = comunLibreria.formatearPeriodoaCadena(periodoIngresado) + "/" + formulario0697.obtenerIdFormulario();
            var dataResult = comunServiciosControlador.obtenerParametrosValidacionFormulario(params);

            var esPerceptorVentasInternas = dataResult.esPerceptorVentasInternas;
            ...
            if (esAplicativoVigente == "1") {
                ...
                if (datepicker < fecIniVig) {
                    form0697.mostrarMensajeGeneral("Sr. Contribuyente, si desea presentar una declaración de período anterior al " + _dataParametriaInicial.fecIniVig + " debe  utilizar el PDT N° 697.");
                } else if (datepicker > fecFinVig) {
                    form0697.mostrarMensajeGeneral("Sr. Contribuyente, el Periodo Ingresado esta fuera del rango de vigencia del formulario 697");
                } else {
                    if (esPerceptorVentasInternas == "1" && esRetenedorPerceptorVigente == "1") {
                        var periodoActual = moment().format("MM/YYYY");
                        if (periodoActual == periodoIngresado) {
                            form0697.mostrarMensajeGeneral("Sr. Contribuyente el período no debe ser mayor o igual al " + periodoActual + " de la fecha actual.");
                        }
                        else {
                            ...
                            formulario0697.obtenerPeriodo();
                            var esReadOnly = form0697.esCasilla404ReadOnly();
                            ...
                            if (retenciones != null && parseInt(retenciones) != 0) {
                                ...
                                if ($('*[data-casilla="895"]').val() == comunLibreria.getEnumTipoDeclaracion().RECTIFICATORIA.value) {
                                    ...
                                    formulario0697.obtenerPagosPrevios(false, false, form0697.obtenerIdFormulario(), '010504');
                                }
                                else {
                                    if (esReadOnly) {
                                        ...
                                        formulario0697.obtenerPagosPrevios(false, true, form0697.obtenerIdFormulario(), '010504');
                        ...
                        form0697.mostrarMensajeGeneral("Sr. Contribuyente, en el período tributario al que corresponde su declaración, Ud. no está designado como Agente de Percepción");
                        $('*[data-casilla="7"]').val("");
                    }
                }
            } else {
                form0697.mostrarMensajeGeneral("Sr. Contribuyente el periodo ingresado  debe ser mayor o igual al periodo de instalación de la plataforma " + comunLibreria.formatearPeriodoSUNAT(dataResult.periodoInicioAplicativo));
                $('*[data-casilla="7"]').val("");
                $('*[data-casilla="895"]').empty();
            ...
            comunServiciosControlador.registrarLogMessage(comunLibreria.generarInformacionLogMessage2(form0697.obtenerNombreJavascript(),"iniciarProcesoObtencionDatos",e));
            throw e;
        }
    ...
    $('*[data-casilla="402"]').click(function () {
        if ($.active > 0) {
            ...
            formulario0697.obtenerPagosPrevios(true, false, form0697.obtenerIdFormulario(), '010504');
        }
    });
    ...
    $('*[data-casilla="404"]').keyup(function () {
        ...
        var periodo = $('*[data-casilla="7"]').val();
        form0697.myIntegrador.setearActivacionCambioCasillas("true==" + periodo);
        var montoCalc = 0;
        ...
        montoCalc = form0697.myIntegrador.obtenerValorCalculoExpresion(control405[0].attributes["data-jsonCalculo"].value);
        ...
        montoCalc = form0697.myIntegrador.obtenerValorCalculoExpresion(control410[0].attributes["data-jsonCalculo"].value);
    ...
    $("#buttonConfirmarSI").click(function () {
        ...
        var periodofinal = $("#casilla7").val();
        localStorage.setItem("periodo_original", periodofinal);
    });
    ...
    $(".barraBuscar02").click(function () {
        ...
        if (!comunLibreria.validarConexionInternet()) {
            ...
            if ($('*[data-casilla="410"]').val() == "") {
                //form0697.mostrarMensajeGeneral("Sr. Contribuyente, Ingrese el importe a pagar.");
            ...
            if (importePagar > deudaTributaria) {
                form0697.mostrarMensajeGeneral("El importe de la casilla 410 no puede ser mayor al importe de la casilla 405.");
                return;
            }

            var messagesArray = form0697.myIntegrador.validarFormularios();
            if (messagesArray.length > 0) {
                $("span.lista-errores-total").text(messagesArray.length);
                $('#lstErrores').empty();
                for (var i = 0; i < messagesArray.length; i++) {
                    $('#lstErrores').append('<li class=\"list-group-item\"><div><span class=\"Errores glyphicon glyphicon-remove\"></span><a href=\"#\" target=\"_self\" onclick="formulario0697.ubicarCasilla(\'' + messagesArray[i] + '\');">Ingresar la casilla ' + messagesArray[i] + '</a></div> </li>');
                }
            ...
            var periodo = $('*[data-casilla="7"]').val();
            if (!comunBandeja.existeFormulario(form0697.obtenerIdFormulario(), periodo)) {
                localStorage.setItem("ventana", "f0697");
                form0697.inactivarGuardadoAutomatico(); //inactiva autoguardado

                form0697.setearValoresAdicionales(); //Seteando valores de casillas ocultas
                ...
                var casillasSugeridas = form0697.obtenerListaCasillasSugeridas();
                ...
                var jsonDetalle = ' "detalle" : { "identificadorFormulario": "##__##__##", "numRuc":"' + numRuc + '", "codFormulario": "' + form0697.obtenerIdFormulario() + '", "numeroVersionFormulario": "' + form0697.obtenerVersionFormulario() + '", "descripcionFormulario": "'+ comunBandeja.obtenerFormularioActivoDescripcion()+'", "montoPago": "' + $('*[data-casilla="410"]').val() + '", "periodoTributo": "' + $('*[data-casilla="7"]').val() + '","rectificatoria" : "' + rectificatoria + '","tasa":' + form0697.obtenerValorTasa() + '}';
                ...
                form0697.myIntegrador.guardarCasillaFrecuente(form0697.obtenerIdFormulario(), JSON.stringify(casillasFrecuentesArray));
                ...
                comunServiciosControlador.registrarLog(comunLibreria.generarInformacionLog("Se agrego formulario 0697 a Bandeja de Presentacion"));
            }
            else {
    ...
    $('#simula-pdf-click').click(function () {
        ...
        var id = form0697.obtenerIdFormulario() + comunLibreria.formatStringPeriod($('*[data-casilla="7"]').val());
    ...
    $("#btnLimpiarFormulario").click(function () {
        ...
        formulario0697.myIntegrador.setearActivacionCambioCasillas("false");
        $('*[data-casilla="895"]').empty();
        $('*[data-casilla="895"]').append("<option value='" + comunLibreria.getEnumTipoDeclaracion().ORIGINAL.value + "'>" + comunLibreria.getEnumTipoDeclaracion().ORIGINAL.name + "</option>");
        $('*[data-casilla="7"]').datepicker("update");
        form0697.limpiarPanelErrores();
        form0697.setearFechaDefecto();
    });
    ...
    $("#tab-li-menu01").click(function (event) {
        ...
        if ($('*[data-casilla="7"]').val() == "") {
            form0697.mostrarMensajeGeneral("Sr. Contribuyente, debe ingresar el período tributario.");
        }
    });
    ...
    $("#tab-li-menu02").click(function (event) {
        ...
        if ($('*[data-casilla="7"]').val() == "") {
            form0697.mostrarMensajeGeneral("Sr. Contribuyente, debe ingresar el periodo tributario.");
        }
    });

    $("#btnNoModificarPeriodoTributario").click(function () {
        var control7 = $('*[data-casilla="7"]');
        var periodoAnterior = form0697.myIntegrador.obtenerPeriodoCambioCasillas();
                ...
                if (arr[i].id == "lstCheck" && arr[i].localName == "input") {
                    if (arr[i].checked) {
                        ...
                        if (parseFloat(valoreditable) <= parseFloat(montopagofinal)) {
                            ...
                            form0697.actualizarPagosPrevios(parseInt(key), "1", parseFloat(valoreditable));
                        ...
                        var key = arr[i].attributes["data-idpagoprevio"].value;
                        var valoreditable = $('*[data-idpagoprevioedit="' + key + '"]').val();
                        form0697.actualizarPagosPrevios(key, "0", parseFloat(valoreditable));
                    }
                }
            }
        }
        comunIntegrador.procesarGuardarPagosPrevios(monto, pagoIds, montoPagos, false, form0697.obtenerIdFormulario(), '010504', calcularInteres);
    ...
    $('#casilla410').keyup(function () {
        var casilla410final = $(this).val() == "" ? 0 : $(this).val();
        $(this).val(casilla410final);
        $('#casilla410').change();
    });
    ...
    $('*[data-casilla="895"]').change(function () {
        form0697.validarDeclaracionSeleccionada();
    });
...
//
//webMessage para el formulario0697
function displayMessageForm0697(evt) {
    console.log("WebMessage:" + evt);
    if (evt.data == "DESACTIVAR-PANEL-PRESENTARPAGAR") {
        $('#myModal-40').modal('hide');
    }
    if (evt.data == "EDITAR-0697") {
        console.log("WebMessage, edicion estando en el mismo formulario");
        var dataJson = comunBandeja.getKeyDataStorage("SUNAT.Edicion.Formulario0697.Data");
        if (dataJson!="") {
...
if (window.addEventListener) {
    window.addEventListener("message", displayMessageForm0697, false);
}
else {
    window.attachEvent("onmessage", displayMessageForm0697);
}

var _edicionForm = comunBandeja.getKeyDataStorage("SUNAT.Edicion.Formulario0697.Accion");
console.log("Verificando si hay edicion 697:" + _edicionForm);
...
function casillasEditarBandeja(dataJsonParse) {
    ...
    if (dataJsonParse != undefined && dataJsonParse != null) {
        ...
        $.each(casillas, function (indexCasilla, itemCasilla) {
            ...
            $.each(dataJsonParse["casillas"], function (index, item) {
                ...
                if (codigoCasilla == codigoCasillaJson && codigoCasillaJson != "7") {
                    formulario0697.cargarComboBox(codigoCasilla, item);
        ...
        var retenciones = $('*[data-casilla="401"]').val();
        $('#inputRentenciones401').val(retenciones);
        localStorage.removeItem('SUNAT.Edicion.Formulario0697.Accion');
        localStorage.removeItem('SUNAT.Edicion.Formulario0697.Data');
    }
}
